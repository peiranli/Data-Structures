...
public class BSTree_EC<T extends Comparable<? super T>>{
	 ...
	 * @param treeChoice if 1 find the root1 if 2 find the roo2 else throw exception
	 * @return the height of a tree
	 * @throws IllegalArgumentException
	 */
	public int findHeight(int treeChoice) throws IllegalArgumentException {
		//get root
		BSTNode root = getRoot(treeChoice);
		if(root == null)
			return -1;
		else
			return height(root);
	...
	private int height(BSTNode n){
		...
		if(n == null)
			return -1;
		//recursion return the max height of two child
		else{
			int leftHeight = height(n.getLChild());
			int rightHeight = height(n.getRChild());
	...
	/**
	 * help method to count leaves
	 * @param n the node to count leaves
	 * @return the count of leaves
	 */
	private int leaf(BSTNode n){
		//base case
		if(n == null)
			return 0;
		//recursion find the left child and right child's leaves
		else{
			if(n.getLChild() == null && n.getRChild() == null)
				return 1;
			else
				return leaf(n.getLChild())+leaf(n.getRChild());
		}
	}

	/**
	 * count the nodes of a given level in a tree
	 * @param level given level
	 * @param treeChoice if 1 find the root1 if 2 find the roo2 else throw exception
	 * @return the count of nodes of a given level
	 * @throws IllegalArgumentException
	 */
	...
	/**
	 * help method to count nodes in a given level
	 * @param n the nodes to count
	 * @param level the given level
	 * @return the count of given level
	 */
	private int count(BSTNode n, int level){
		//base case
		if(n == null)
			return 0;
		else{
			...
			if(level == 0)
				return 1;
			//recursion count left tree and right tree
			else 
				return count(n.getLChild(),level-1)+count(n.getRChild(),level-1);
		}
	}


	/**
	 * load trees from a given file path
	 * @param filePath the file path that stores trees
	 */
	...
	public void loadData(String filePath) {
		...
		try{
			...
			while(input.hasNext()){
				T firstName = (T) input.next();
				T secondName = (T) input.next();
				insert(firstName,secondName);
			}
			input.close();
	...
	public void saveData(String filePath) {
		...
		try{
			...
			//remove an element and explore all its children
			while(q.size() != 0){
				T first = q.remove();
				output.print(first+" "+findMoreInfo(first)+"\n");
				BSTNode removed = find(root1,first);
				if(removed.getLChild() != null && !q.contains(removed.getLChild()))
					q.add(removed.getLChild().getElem());
				if(removed.getRChild() != null && !q.contains(removed.getRChild()))
					q.add(removed.getRChild().getElem());
			}
			output.close();
		}catch(FileNotFoundException ex){
			System.out.println("cannot load file");
